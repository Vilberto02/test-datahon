...
model = joblib.load("Gold_Price_Prediction_Model.pkl")

# Cargar los datos nuevamente (los mismos utilizados para el entrenamiento)
file_path = "Selected_Features_Gold_Price.csv"
...
def filter_heatmap(cols):
  ...
  fig = px.imshow(
      ...
      color_continuous_scale='RdBu_r',  # Esquema de color mejorado
      zmin=-1, zmax=1,  # Limitar el rango de la escala de colores a -1 y 1
      labels=dict(x="Variables", y="Variables", color="Correlación"),  # Etiquetas personalizadas
  )

  # Mejorar la presentación
  fig.update_layout(
      xaxis={'side': 'bottom'},  # Coloca las etiquetas del eje X en la parte inferior
      yaxis_autorange='reversed',  # Revertir el eje Y para una mejor visualización
      width=800, height=600,  # Tamaño del gráfico
...
def upgrade_graph_scatter(x_axis):
  fig = px.scatter(
    data_frame=data,
    x=x_axis,
    y='Price Tomorrow',  # Suponiendo que esta es la variable dependiente
